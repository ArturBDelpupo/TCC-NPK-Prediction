# ==========================================================
# LUCAS SOIL — SISTEMA NPK (VERSÃO COM KNN, MLP E STACKING NÃO LINEAR)
# ==========================================================
...
print("=" * 90)
print("🚀 SISTEMA NPK — COM KNN, MLP E STACKING NÃO LINEAR")
print("=" * 90)
...
if usar_filtro:
    ...
    usar_outliers = perguntar("Remover outliers extremos (NPK)?")  # não usado devido à winsorização
    ...
    if usar_textura and "OLM_TextureClass" in df.columns:
        ...
        if alvo_textura:
            try:
                ...
                pass
    if usar_producao and "LC1_num" in df.columns:
        df = df.dropna(subset=["LC1_num"])
        alvo_producao = input("\n  👉 Digite as vegetacao para manter (ex: 0.043(Olive groves), 0.027(Potatoes)): ").strip()
        if alvo_producao:
            try:
                lista_alvos = [float(x.strip()) for x in alvo_producao.split(",")]
                df = df[df["LC1_num"].isin(lista_alvos)]
                print(f"  ✅ Filtro aplicado")
            except ValueError:
                pass
else:
...
##################################################################################################33
# ==========================================================
# 3. ESTATÍSTICAS DESCRITIVAS DE N, P, K NA BASE FINAL
# ==========================================================

print("\n" + "=" * 80)
print("📌 ESTATÍSTICAS DA BASE FINAL (APÓS FILTROS)")
print("=" * 80)
...
for nut in ["N", "P", "K"]:
    ...
    serie = pd.to_numeric(df[nut], errors="coerce").dropna()

    if len(serie) == 0:
        print(f"\n⚠️ Nutriente {nut}: sem dados válidos após filtros.")
        continue
...
# Salvar em CSV para usar na tese
summary_df = pd.DataFrame(resumo)
summary_df.to_csv("summary_datasetA_NPK_final.csv", index=False)
print("\n✅ Estatísticas salvas em 'summary_datasetA_NPK_final.csv'.")

# Encerrar o script aqui (não executar mais nada de ML)
import sys
sys.exit(0)

# ==========================================================
# 4. EXTRAÇÃO DE FEATURES FENOLÓGICAS E CLIMA
# ==========================================================
print("\n" + "=" * 80)
...
# ==========================================================
# 5. FÓRMULAS AGRONÔMICAS
# ==========================================================
print("\n[5] Criando interações agronômicas...")
...
# ==========================================================
# 6. MATRIZ DE FEATURES, REMOÇÃO DE CORRELACIONADAS, PCA
# ==========================================================
...
print("\n[6] Montando Matriz, PCA e Split Treino/Teste (80/20)...")
...
print("   [6.1] Removendo features com correlação > 0.95...")
tmp = df[features_base].dropna()
corr_matrix = tmp.corr().abs()
upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))

# No bloco 6.1, mude para:
...
# ==========================================================
# 7. CLASSES NPK E IMPUTAÇÃO
# ==========================================================
print("\n[7] Criando classes balanceadas (tercis fixos do treino)...")
...
print("   [7.1] Aplicando padronização global nas features...")
scaler_global = StandardScaler()
X_train_full = scaler_global.fit_transform(X_train_full)
X_test_full = scaler_global.transform(X_test_full)
print("   ✅ Features padronizadas (média 0, std 1).")

# ==========================================================
# 7.6 OTIMIZAÇÃO DE HIPERPARÂMETROS DA MLP
# ==========================================================
print("\n" + "=" * 80)
print("🔍 OTIMIZANDO HIPERPARÂMETROS DA MLP (AMOSTRA PARA N)")
print("=" * 80)
...
print(f"✅ Melhores parâmetros encontrados para MLP:")
for k, v in random_search.best_params_.items():
    print(f"   - {k}: {v}")
print(f"✅ Melhor R² (CV): {random_search.best_score_:.4f}\n")

# ==========================================================
# 8. FUNÇÕES PARA O LOOP RFE-SHAP (COM MODELOS AUMENTADOS)
# ==========================================================
...
def treinar_e_avaliar(dict_features, iteracao_nome, primeira_iteracao=False):
    print(f"\n⚙️  Iniciando etapa: {iteracao_nome}")
    ...
    # ==========================================================
    # REGRESSÃO BASE COM K-FOLD
    # ==========================================================
    print("\n   [->] Modelos de REGRESSÃO (10-fold CV nos modelos base)...")
    modelos_reg = {
        ...
        "MLP": MLPRegressor(
            hidden_layer_sizes=(60, 25), activation='relu', solver='adam',
    ...
    for nome, modelo_template in modelos_reg.items():
        for i, nut in enumerate(["N", "P", "K"]):
            ...
            y_nut_log_tr = y_reg_log[idx_train][mask_tr, i]
            y_nut_orig_tr = y_reg_original[idx_train][mask_tr, i]

            m_cloned = clone(modelo_template)

            preds_log_oof = cross_val_predict(
                m_cloned,
                X_nut_tr,
                y_nut_log_tr,
                cv=kf,
            ...
            preds_orig_oof = np.expm1(preds_log_oof)

            r2 = r2_score(y_nut_orig_tr, preds_orig_oof)
            mae = mean_absolute_error(y_nut_orig_tr, preds_orig_oof)
            rmse = np.sqrt(mean_squared_error(y_nut_orig_tr, preds_orig_oof))

            resultados_iteracao.append({
                "Filtro": iteracao_nome,
                "Modelo": f"Reg_{nome}_CV",
                "Nutriente": nut,
                "R2": round(r2, 3),
                "MAE": round(mae, 2),
                "RMSE": round(rmse, 2),
            ...
            # Treinar CatBoost no treino completo para SHAP
            if nome == "CatBoost":
                modelo_shap = clone(modelo_template)
                modelo_shap.fit(X_nut_tr, y_nut_log_tr)
                cat_treinados_reg[nut] = modelo_shap

    # ==========================================================
    # CLASSIFICAÇÃO BASE COM K-FOLD
    # ==========================================================
    print("   [->] Modelos de CLASSIFICAÇÃO (10-fold CV nos modelos base)...")
    modelos_clf = {
        ...
        "CatBoost": FlatCatBoostClassifier(
            iterations=500, learning_rate=0.03, depth=6,
    ...
    for nome, modelo_template in modelos_clf.items():
        for i, nut in enumerate(["N", "P", "K"]):
            ...
            X_nut_tr = X_tr_dict[nut][mask_tr]
            y_nut_tr = y_train_clf[mask_tr, i]

            m_cloned = clone(modelo_template)

            if nome == "XGBoost":
                preds_oof = np.zeros(len(y_nut_tr), dtype=int)

                for fold_id, (tr_idx, val_idx) in enumerate(kf.split(X_nut_tr), start=1):
                    X_fold_tr, X_fold_val = X_nut_tr[tr_idx], X_nut_tr[val_idx]
                    y_fold_tr = y_nut_tr[tr_idx]

                    from sklearn.utils.class_weight import compute_class_weight
                    classes = np.unique(y_fold_tr)
                    weights = compute_class_weight('balanced', classes=classes, y=y_fold_tr)
                    sample_weights = np.zeros_like(y_fold_tr, dtype=float)
                    for cls, w in zip(classes, weights):
                        sample_weights[y_fold_tr == cls] = w

                    modelo_fold = clone(modelo_template)
                    modelo_fold.fit(X_fold_tr, y_fold_tr, sample_weight=sample_weights)
                    preds_oof[val_idx] = modelo_fold.predict(X_fold_val)
            ...
            acc = accuracy_score(y_nut_tr, preds_oof)
            f1 = f1_score(y_nut_tr, preds_oof, average='weighted')

            resultados_iteracao.append({
                "Filtro": iteracao_nome,
                "Modelo": f"Clf_{nome}_CV",
                "Nutriente": nut,
                "R2": np.nan,
                "MAE": np.nan,
                "RMSE": np.nan,
            ...
            if primeira_iteracao:
                dict_clf_preds[nut][nome] = (y_nut_tr, preds_oof, acc)

    # ==========================================================
    # STACKING NÃO LINEAR (mantido com teste cego)
    # ==========================================================
    print("   [->] STACKING (Avaliando no Teste Cego) com meta-modelo XGBoost...")
    ...
    if primeira_iteracao:
        ...
        for nut_i, nut in enumerate(["N", "P", "K"]):
            ...
            y_true_cv, y_pred_cv, _ = dict_clf_preds[nut][melhor_nome]
            cm = confusion_matrix(y_true_cv, y_pred_cv)

            sns.heatmap(
                cm, annot=True, fmt='d', cmap=cm_cmaps[nut_i], cbar=False, ax=ax,
                xticklabels=["Baixa", "Média", "Alta"],
                yticklabels=["Baixa", "Média", "Alta"],
                annot_kws={"weight": "bold", "size": 14}
            )
            ax.set_xlabel(f"Modelo {melhor_nome} (CV)")
            ax.set_ylabel("Treino (OOF)")
            plt.tight_layout()
            nome_cm = f"matriz_confusao_cv_{nut}.png"
...
def calcular_shap_individual(cat_dict, dict_features):
    print("\n   🔍 Calculando SHAP para extrair os próximos TOPs INDIVIDUAIS...")
    ranking_dict = {}
...
# ==========================================================
# 9. PIPELINE DE SELEÇÃO ESTÁTICA (RANKING GLOBAL) - INALTERADO
# ==========================================================
print("\n" + "=" * 80)
print("🌀 CALCULANDO RANKING GLOBAL SHAP (ÂNCORA) COM FEATURES CORRELACIONADAS REMOVIDAS")
print("=" * 80)

features_atuais_todas = {"N": features_base.copy(), "P": features_base.copy(), "K": features_base.copy()}
_, cat_treinados_mestre = treinar_e_avaliar(features_atuais_todas, "Cálculo do Ranking Mestre")

rankings_mestres = calcular_shap_individual(cat_treinados_mestre, features_atuais_todas)

print("\n" + "=" * 80)
print("🌀 INICIANDO TESTES POR TAMANHO DE SUBSET (TOP 50, 25, 10, 5)")
print("=" * 80)
...
for iteracao_nome, corte in etapas_fixas:
    features_da_rodada = {}
    for nut in ["N", "P", "K"]:
        features_da_rodada[nut] = rankings_mestres[nut]["Feature"].head(corte).tolist()
    ...
    print(f"\n👑 FEATURES UTILIZADAS NESTA ETAPA ({iteracao_nome}):")
    for nut in ["N", "P", "K"]:
        print(f"\n   ► Top {corte} para {nut}:")
        print(rankings_mestres[nut].head(corte).to_string(index=False))

# ==========================================================
# DISTRIBUIÇÃO DAS CLASSES NO TREINO (N, P, K)
# ==========================================================
print("\n" + "=" * 80)
print("📌 DISTRIBUIÇÃO DAS CLASSES (BAIXA / MÉDIA / ALTA) NO TREINO")
print("=" * 80)

rotulos = {0: "Baixa", 1: "Média", 2: "Alta"}

for i, nut in enumerate(["N", "P", "K"]):
    valores, contagens = np.unique(y_train_clf[:, i], return_counts=True)
    total = contagens.sum()
    print(f"\nNutriente {nut} (n = {total} no treino):")
    for v, c in zip(valores, contagens):
        nome = rotulos.get(v, str(v))
        perc = 100 * c / total
        print(f"   {nome:5s}: {c:4d} amostras ({perc:5.1f}%)")        

# ==========================================================
# BASELINE DO DATASET: MAE E RMSE AO PREVER A MÉDIA DO TREINO
# ==========================================================
print("\n" + "=" * 80)
print("📌 BASELINE: ERRO AO PREVER SEMPRE A MÉDIA DO TREINO (N, P, K)")
print("=" * 80)

for i, nut in enumerate(["N", "P", "K"]):
    # valores reais (originais, não log) de treino e teste
    y_tr = y_reg_original[idx_train, i]
    y_te = y_reg_original[idx_test, i]
    ...
    # predição baseline = média do treino
    baseline = np.full_like(y_te, fill_value=y_tr.mean(), dtype=float)

    mae_base = mean_absolute_error(y_te, baseline)
    rmse_base = np.sqrt(mean_squared_error(y_te, baseline))

    print(f"\nNutriente {nut}:")
    print(f"   Média treino      : {y_tr.mean():.3f}")
    print(f"   Baseline MAE (test): {mae_base:.3f}")
    print(f"   Baseline RMSE (test): {rmse_base:.3f}")

# ==========================================================
# ==========================================================
# NOVO: HISTOGRAMAS TREINO/TESTE – STACKING COM TOP 25 FEATURES
# ==========================================================
print("\n" + "=" * 80)
print("📊 GERANDO HISTOGRAMAS TREINO/TESTE PARA STACKING COM TOP 25 FEATURES")
print("=" * 80)

# 1. Selecionar as 25 features mais importantes para cada nutriente
top25_features = {}
for nut in ["N", "P", "K"]:
    top25_features[nut] = rankings_mestres[nut]["Feature"].head(25).tolist()

# 2. Preparar os dados com essas features
X_tr_top25 = {}
X_te_top25 = {}
for nut in ["N", "P", "K"]:
...
# 3. Definir os modelos base do stacking (iguais aos usados na função treinar_e_avaliar)
modelos_stack_hist = {
...
for nut in ["N", "P", "K"]:
    ...
    # 4. Criar figura com dois painéis: reais e predições
    fig, axes = plt.subplots(2, 1, figsize=(10, 8))

    # Painel superior: distribuição dos valores REAIS (Treino vs Teste)
    ...
    # Painel inferior: distribuição das PREDIÇÕES (Treino vs Teste)
    ...
    plt.tight_layout()
    nome_hist = f"histograma_treino_teste_{nut}_Top25.png"
    plt.savefig(nome_hist, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"   ✅ Histograma treino/teste para {nut} salvo em '{nome_hist}'")

# ==========================================================
# 10. EXIBIÇÃO DA TABELA FINAL MESTRA
# ==========================================================
print("\n" + "=" * 80)
print("🏆 TABELA MESTRA DE RESULTADOS (NOTAS ESTRITAS DO TESTE CEGO)")
print("=" * 80)
...
tabela_geral_resultados.to_csv("TCC_Resultados_RFE_TesteCego_COM_KNN_MLP.csv", index=False, sep=";", decimal=",")
print("\n✅ Tabela exportada com sucesso para: 'TCC_Resultados_RFE_TesteCego_COM_KNN_MLP.csv'")
print("=" * 80)